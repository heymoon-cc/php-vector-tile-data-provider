...
composer: clean.container
	docker run --name mvt-tools-tester -v $$(pwd):/code heymoon/mvt-tools-tester install

audit: clean.container
	docker run --name mvt-tools-tester -v $$(pwd):/code heymoon/mvt-tools-tester test

phpmd: clean.container
	docker run --name mvt-tools-tester -v $$(pwd):/code heymoon/mvt-tools-tester phpmd

clean.container:
	docker rm mvt-tools-tester 2> /dev/null || true
